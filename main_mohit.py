...
def login_screen():
    ...
    Label(login, text="Enter password:", font=("calibri", 13)).place(x=10, y=10)

    password = StringVar()
    Entry(login, textvariable=password, width=15, bd=0, font=("arial", 15), show="*").place(x=10, y=40)

    Button(login, text="Login", height="1", width="8", bg="#00bd56", fg="white", bd=0, command=lambda: check_password(password.get())).place(x=10, y=80)

    login.mainloop()

def check_password(input_password):
    if input_password == "mohit@123":
        login.destroy()
        main_screen()
    else:
